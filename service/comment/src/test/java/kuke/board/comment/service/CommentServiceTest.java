...
class CommentServiceTest {
    ...
    @Test
    @DisplayName("하위 댓글이 삭제되고, 삭제되지 않은 부모면, 하귀 댓글만 삭제한다.")
    void deleteShouldDeleteChildOnlyIfNotDeletedParent() {
        ...
        Comment parentCommnet = mock(Comment.class);
        given(parentCommnet.getDeleted()).willReturn(false);

        given(commentRepository.findById(commentId)).willReturn(Optional.of(comment));
        given(commentRepository.countBy(articleId, commentId, 2L)).willReturn(1L);

        given(commentRepository.findById(parentCommentId)).willReturn(Optional.of(parentCommnet));
        ...
        verify(commentRepository, never()).delete(parentCommnet);
    }
    ...
    void deleteShouldDeleteAllRecursivelyIfDeletedParent() {
        ...
        Comment parentCommnet = createComment(articleId, parentCommentId);
        given(parentCommnet.isRoot()).willReturn(true);
        given(parentCommnet.getDeleted()).willReturn(true);

        given(commentRepository.findById(commentId)).willReturn(Optional.of(comment));
        given(commentRepository.countBy(articleId, commentId, 2L)).willReturn(1L);

        given(commentRepository.findById(parentCommentId))
                .willReturn(Optional.of(parentCommnet));
        ...
        // then
        verify(commentRepository).delete(comment);
        verify(commentRepository).delete(parentCommnet);
    }
